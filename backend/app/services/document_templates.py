...
def render_notice_docx(payload: dict[str, Any], agency_name: str = "主管税务机关") -> bytes:
    ...
    notice_no = f"税通〔{datetime.now().year}〕{payload.get('task_id', '0000')[-6:]}号"
    ...
    _add_paragraph(doc, _chinese_date(), align=WD_ALIGN_PARAGRAPH.CENTER, size=16)
...
def render_officer_report_docx(payload: dict[str, Any]) -> bytes:
    ...
    _add_paragraph(doc, f"报告编号：{payload.get('task_id', '待生成')}", align=WD_ALIGN_PARAGRAPH.CENTER, size=14)
    _add_paragraph(doc, f"生成日期：{datetime.now().strftime('%Y-%m-%d')}", align=WD_ALIGN_PARAGRAPH.CENTER, size=14)
    _add_paragraph(doc)
    ...
    for index, risk in enumerate(risks, start=1):
        ...
        _add_paragraph(doc, f"涉及数据：{_safe_join(risk.get('evidence', []))}", size=16, first_indent=True)
